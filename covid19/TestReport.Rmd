...
```{r echo=FALSE}
CountyChoro = PlotLocalChoro(Counties, base, "County")
CountyChoro
```
...
CCDaily = CovidCasesPerDayChart(base, radius, Counties, Hospitals)
CCDaily = CCDaily %>% layout(xaxis = list(showgrid = F),
...
<div class = "col-md-6">
```{r fig.height = 4, fig.width = 4, echo=FALSE}
CCtot = CovidCasesCumChart(base, radius, Counties, Hospitals)
CCtot = CCtot %>% layout(xaxis = list(showgrid = F),
                 yaxis = list(gridcolor = "lightgray"))
CCtot
```
</div>
...
Base is `r params$base` and the radius is `r params$radius`
